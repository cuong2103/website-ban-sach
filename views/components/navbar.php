  ...
  <div class="max-w-[1200px] mx-auto px-4">
    <div class="flex items-center gap-4 h-16">
      ...
      <form action="<?= BASE_URL ?>?act=books" method="GET" class="flex-1 max-w-xl">
        ...
        <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden hover:border-[#4CAF50] transition-colors">
          <input
            type="text"
            name="search"
          ...
          >
          <button type="submit" class="px-4 py-2 bg-[#4CAF50] text-white hover:bg-[#43A047] transition-colors">
      ...
      <div class="hidden md:flex items-center gap-1">
        ...
        <div class="relative group">
          <button class="flex items-center gap-1 px-3 py-2 text-sm text-[#333] hover:text-[#4CAF50] hover:bg-gray-50 rounded-lg transition-colors">
            Danh mục <i data-lucide="chevron-down" class="w-[14px] h-[14px]"></i>
          </button>
          <div class="absolute top-full left-0 bg-white shadow-lg rounded-xl py-2 min-w-48 border border-gray-100 z-50 hidden group-hover:block">
            <?php
            $categories = [
              ['icon' => '📖', 'name' => 'Văn học'],
              ['icon' => '💼', 'name' => 'Kinh tế'],
      ...
      <div class="flex items-center gap-2 ml-auto md:ml-0">
        ...
        <a href="<?= BASE_URL ?>?act=cart"
          ...
          <span class="absolute -top-1 -right-1 bg-[#FFC107] text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-medium hidden" id="cart-badge">0</span>
          <span class="hidden sm:block">Giỏ hàng</span>
        </a>

        <!-- Mobile hamburger -->
        <button class="md:hidden p-2 hover:bg-gray-50 rounded-lg" onclick="document.getElementById('mobile-menu').classList.toggle('hidden')">
    ...
    <div id="mobile-menu" class="md:hidden hidden border-t border-gray-100 py-3 space-y-1">
      ...
      <?php endforeach; ?>
      <a href="<?= BASE_URL ?>?act=account" class="flex items-center gap-2 px-4 py-2 text-sm text-[#333] hover:bg-gray-50 rounded-lg">
