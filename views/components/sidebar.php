...
function isActive($acts, $currentAct) {
  if (is_array($acts)) return in_array($currentAct, $acts);
...
<aside class="w-56 bg-[#1B2537] text-white flex-shrink-0 flex flex-col h-screen fixed inset-y-0 left-0 z-50">
  ...
  <nav class="flex-1 overflow-y-auto py-3 space-y-0.5">
    ...
    <!-- Dashboard -->
    <a href="<?= BASE_URL ?>?act=admin-dashboard" class="flex items-center gap-3 px-4 py-2.5 mx-2 rounded-xl text-sm transition-colors <?= isActive('', $currentAct) ? $activeClass : $inactiveClass ?>">
  ...
  <div class="p-3 border-t border-white/10">
    <a href="<?= BASE_URL ?>?act=logout" class="flex items-center gap-3 px-3 py-2 rounded-xl text-gray-400 hover:bg-white/10 hover:text-white text-sm transition-colors">
