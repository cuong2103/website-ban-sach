...
<div class="relative overflow-hidden" id="banner-slider">
  ...
  foreach ($banners as $i => $b): ?>
    <div
      class="banner-slide absolute inset-0 h-64 md:h-80 flex flex-col justify-center px-6 md:px-16 transition-opacity duration-500 <?= $i === 0 ? 'opacity-100 relative' : 'opacity-0 pointer-events-none' ?>"
      ...
      <a href="<?= BASE_URL ?>?act=books"
        class="inline-block bg-[#FFC107] hover:bg-[#FFB300] text-[#333] px-6 py-2.5 rounded-lg w-fit font-semibold transition-colors">
        <?= $b['cta'] ?>
      </a>
    </div>
  <?php endforeach; ?>
  ...
  <button id="banner-prev"
    class="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors z-10">
    <i data-lucide="chevron-left" class="w-[18px] h-[18px]"></i>
  </button>
  <button id="banner-next"
    class="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors z-10">
  ...
  <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10" id="banner-dots">
    <?php for ($i = 0; $i < count($banners); $i++): ?>
      <button
        class="banner-dot w-2 h-2 rounded-full transition-all <?= $i === 0 ? 'bg-white w-6' : 'bg-white/50' ?>"></button>
    <?php endfor; ?>
  </div>
</div>
...
<div class="max-w-[1200px] mx-auto px-4 py-8 space-y-12">
    ...
    <div class="bg-white rounded-b-xl p-5 shadow-sm">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        foreach ($saleBooks as $book): ?>
          <div
            class="group relative bg-white border border-gray-100 rounded-xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
            ...
            <div class="p-3">
              ...
              <button
                class="mt-2 w-full py-1.5 bg-[#4CAF50] hover:bg-[#43A047] text-white text-xs rounded-lg transition-colors">
                Thêm giỏ hàng
              </button>
            </div>
          </div>
  ...
  <section>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      foreach ($newBooks as $book): ?>
        <div
          class="group bg-white border border-gray-100 rounded-xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
          <div class="relative h-40 bg-gradient-to-br from-green-50 to-green-100 flex items-center justify-center">
            <span
              class="absolute top-2 left-2 bg-[#4CAF50] text-white text-xs font-bold px-2 py-0.5 rounded-full"><?= $book['tag'] ?></span>
          ...
          <div class="p-3">
            ...
            <span class="text-sm font-bold text-[#4CAF50]"><?= $book['price'] ?></span>
            <button
              class="mt-2 w-full py-1.5 border border-[#4CAF50] text-[#4CAF50] hover:bg-[#4CAF50] hover:text-white text-xs rounded-lg transition-colors">
              Thêm giỏ hàng
            </button>
          </div>
        </div>
      ...
      foreach ($hotBooks as $book): ?>
        <div
          ...
          <div class="p-3">
            ...
            <span class="text-sm font-bold text-[#4CAF50]"><?= $book['price'] ?></span>
            <button
              class="mt-2 w-full py-1.5 bg-[#FFC107] hover:bg-[#FFB300] text-[#333] text-xs rounded-lg font-medium transition-colors">
              Thêm giỏ hàng
            </button>
          </div>
        </div>
  ...
  <section>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
      <?php
      $categoryIcons = [
        'van-hoc' => ['icon' => '📖', 'color' => 'from-blue-50 to-blue-100', 'border' => 'border-blue-200'],
        'kinh-te' => ['icon' => '💼', 'color' => 'from-green-50 to-green-100', 'border' => 'border-green-200'],
        'thieu-nhi' => ['icon' => '🧒', 'color' => 'from-yellow-50 to-yellow-100', 'border' => 'border-yellow-200'],
        'ky-nang-song' => ['icon' => '🌟', 'color' => 'from-purple-50 to-purple-100', 'border' => 'border-purple-200'],
        'khoa-hoc' => ['icon' => '🔬', 'color' => 'from-cyan-50 to-cyan-100', 'border' => 'border-cyan-200'],
        'lich-su' => ['icon' => '📜', 'color' => 'from-orange-50 to-orange-100', 'border' => 'border-orange-200'],
      ];
      $defaultIcon = ['icon' => '📚', 'color' => 'from-indigo-50 to-indigo-100', 'border' => 'border-indigo-200'];

      if (!empty($categories)):
        foreach ($categories as $cat):
          $icons = $categoryIcons[$cat['slug']] ?? $defaultIcon;
          ?>
          <a href="<?= BASE_URL ?>books?category=<?= urlencode($cat['slug']) ?>"
            class="flex items-center gap-4 p-5 bg-gradient-to-r <?= $icons['color'] ?> border <?= $icons['border'] ?> rounded-xl hover:shadow-md transition-shadow">
            <span class="text-3xl"><?= $icons['icon'] ?></span>
            <div>
              <p class="font-semibold text-[#333]"><?= htmlspecialchars($cat['name']); ?></p>
  ...
  <!-- Email signup banner -->
  <section
    class="rounded-xl overflow-hidden bg-gradient-to-r from-[#4CAF50] to-[#1B5E20] p-8 flex flex-col md:flex-row items-center justify-between gap-6">
    ...
    <div class="flex gap-2 w-full md:w-auto">
      ...
      <button
        class="bg-[#FFC107] hover:bg-[#FFB300] text-[#333] px-5 py-2.5 rounded-lg font-semibold transition-colors whitespace-nowrap">
        Đăng ký
      </button>
    </div>
  </section>
